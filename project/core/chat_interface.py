...
from rag_agent.query_router import format_route_display

SILENT_NODES = {"rewrite_query", "route_query"}
SYSTEM_NODES = {"summarize_history", "rewrite_query", "route_query"}

SYSTEM_NODE_CONFIG = {
    "route_query":       {"title": "🔀 查询路由"},
...
class ChatInterface:
    ...
    def _handle_route_update(self, output, response_messages):
        route = output.get("query_route", "")
        if not route:
            return

        content = format_route_display(
            route,
            output.get("route_source", ""),
            float(output.get("route_confidence", 0.0)),
            output.get("route_reason", ""),
        )
        idx = find_msg_idx(response_messages, "route_query")
        if idx is None:
            response_messages.append(make_message(content, title="🔀 查询路由", node="route_query"))
        else:
            response_messages[idx]["content"] = content
        ...
        if route == "clarify":
            ...
            if messages:
                clarification = getattr(messages[-1], "content", "") or ""
    ...
    def chat(self, message, history, *, force_rag=False):
        ...
        try:
            if current_state.next:
                self.rag_system.agent_graph.update_state(config, payload)
                stream_input = None
            else:
            ...
            for mode, chunk in self.rag_system.agent_graph.stream(
                stream_input,
                config=config,
                ...
                if mode == "updates":
                    for node_name, node_output in chunk.items():
                        if node_name == "route_query" and isinstance(node_output, dict):
                            self._handle_route_update(node_output, response_messages)
                ...
                if node in SYSTEM_NODES and isinstance(message_chunk, AIMessageChunk) and message_chunk.content:
                    self._handle_system_node(message_chunk, node, response_messages, system_node_buffer)

                elif hasattr(message_chunk, "tool_calls") and message_chunk.tool_calls:
                    self._handle_tool_call(message_chunk, response_messages, active_tool_calls)

                elif isinstance(message_chunk, ToolMessage):
                    self._handle_tool_result(message_chunk, response_messages, active_tool_calls)

                elif isinstance(message_chunk, AIMessageChunk) and message_chunk.content and node not in SILENT_NODES:
                    self._handle_llm_token(message_chunk, node, response_messages)
